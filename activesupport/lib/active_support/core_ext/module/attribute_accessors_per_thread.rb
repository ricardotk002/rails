...
class Module
  ...
  def thread_mattr_reader(*syms, instance_reader: true, instance_accessor: true, default: nil)
    syms.each do |sym|
      ...
      Thread.current["attr_" + name + "_#{sym}"] = sym_default_value unless sym_default_value.nil?
    end
  end
  ...
  def thread_mattr_writer(*syms, instance_writer: true, instance_accessor: true,  default: nil)
    syms.each do |sym|
      raise NameError.new("invalid attribute name: #{sym}") unless /^[_A-Za-z]\w*$/.match?(sym)
